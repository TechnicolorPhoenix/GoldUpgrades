...
import com.titanhex.thex.util.EnergyCapabilityProviderItem;
import com.titanhex.thex.util.PowerUnit;
...
public class FEPoweredShieldItem extends ShieldBaseItem implements IDamageShield, IItemPoweredFE
{
    ...
    {
    	tooltip.add(new TranslationTextComponent("tooltip." + ModThexShields.ID + "." + preferredEnergyUnit.getCapacityTranslationKey(), MathHelper.floor(this.getFEStored(stack) * preferredEnergyUnit.getEnergyScaleToFE()), MathHelper.floor(this.getFECapacity(stack)  * preferredEnergyUnit.getEnergyScaleToFE())));
    	tooltip.add(new TranslationTextComponent("tooltip." + ModThexShields.ID + "." + preferredEnergyUnit.getEnergyChargeRateTranslationKey(), MathHelper.floor(this.maxEnergyReceive * preferredEnergyUnit.getEnergyScaleToFE())));
    	tooltip.add(new TranslationTextComponent("tooltip." + ModThexShields.ID + "." + preferredEnergyUnit.getEnergyPerDamageTranslationKey(), MathHelper.floor(Config.INSTANCE.damageToFEMultiplier.get() * 2 * preferredEnergyUnit.getEnergyScaleToFE())));
    	tooltip.add(new TranslationTextComponent("tooltip." + ModThexShields.ID + "." + "fe_shield.desc"));
        ...
        else
        {
        	if(worldIn.isClientSide)
        		playerIn.sendMessage(new StringTextComponent(TextFormatting.YELLOW
        			+ I18n.get("message." + ModThexShields.ID + ".powered_shield_block_fail", stack.getDisplayName().getString())), Util.NIL_UUID);
