...
public class BasicShieldItem extends ShieldBaseItem
{
    ...
    {
		tooltip.add(new TranslationTextComponent("tooltip." + ModThexShields.ID + ".protection", this.getMaxDamage(stack)));
    	
    	if(!stack.isEmpty() && stack.hasTag() && stack.getTag().contains("BlockEntityTag"))
    	{
    		DyeColor dyeColor = ShieldItem.getColor(stack);
    		tooltip.add(new StringTextComponent(""));
    		tooltip.add(new TranslationTextComponent("tooltip." + ModThexShields.ID + ".has_patterns"));
